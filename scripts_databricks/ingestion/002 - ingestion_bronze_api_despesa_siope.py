...
api_url = f"https://www.fnde.gov.br/olinda-ide/servico/DADOS_ABERTOS_SIOPE/versao/v1/odata/Despesas_Siope(Ano_Consulta=@Ano_Consulta,Num_Peri=@Num_Peri,Sig_UF=@Sig_UF)?@Ano_Consulta={ano_consulta}&@Num_Peri={num_peri}&@Sig_UF='{sig_uf}'&$format=json"
...
if data:
    ...
    schema = StructType([
        ...
        StructField("NOM_PAST", StringType(), True),
        StructField("IDN_EXIB_CODI", StringType(), True),
        StructField("COD_PAST", StringType(), True),
        StructField("COD_SUBF", StringType(), True),
        StructField("TIP_PASTA", StringType(), True),
        StructField("COD_EXIB", StringType(), True),
        StructField("COD_EXIB_FORMATADO", StringType(), True),
        StructField("COD_FONTE", StringType(), True),
    ...
    df = df_json\
        ...
        .format("delta")\
        .mode("overwrite")\
        .option("path", "/mnt/bronze/tb_despesa_siope")\
        .saveAsTable("bronze.tb_despesa_siope")
...
#spark.sql("""SELECT * FROM bronze.tb_despesa_siope""").display()